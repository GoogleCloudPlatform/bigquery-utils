...
public class ErrorRecoveryTest {
  ...
  @Test
  public void expectedTokenFilter() {
    String query = "SELECT a WHERE b";
    Parser parser = new CalciteParser();
    try {
      parser.parseQuery(query);
      ...
      ArrayList<ReplacedComponent> actual =  QueryBreakdown.replacement(query, 3,
          ...
          ((SqlParseException) e).getPos().getEndColumnNum(),
          test);
      assertFalse(actual.get(0).getReplacement().charAt(0) == '\"');
      if (actual.get(0).getReplacement().length() >= 2) {
        assertFalse(actual.get(0).getReplacement().charAt(1) == '<');
        assertFalse(actual.get(0).getReplacement()
            .charAt(actual.get(0).getReplacement().length() - 1) == '\"');
        assertFalse(actual.get(0).getReplacement()
            .charAt(actual.get(0).getReplacement().length() - 2) == '>');
      }
    }
  }
  ...
  public void replacementSingleLine() {
    ...
    try {
      ...
      Collection<String> test = ((SqlParseException) e).getExpectedTokenNames();
      ArrayList<ReplacedComponent> actual =  QueryBreakdown.replacement(query, 1,
          ((SqlParseException) e).getPos().getLineNum(),
          ((SqlParseException) e).getPos().getColumnNum(),
          ((SqlParseException) e).getPos().getEndLineNum(),
          ((SqlParseException) e).getPos().getEndColumnNum(),
          test);
      assertEquals("WHERE", actual.get(0).getOriginal());
      assertEquals("SELECT a " + actual.get(0).getReplacement() + " b",
          actual.get(0).getQuery());
    }
  }

  @Test
  public void replacementThreeOptions() {
    String query = "SELECT a WHERE b";
    Parser parser = new CalciteParser();
    try {
      parser.parseQuery(query     );
