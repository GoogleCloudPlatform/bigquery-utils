...
-- @param STRING name of the column in our table that represents the initial observation variable
-- @param STRING name of the column in our table that represents the final (paired) observation variable
-- @return STRUCT<t_value FLOAT64, df FLOAT64, p_value FLOAT64>

CREATE OR REPLACE PROCEDURE bqutil.procedure.t_test_paired (table_name STRING, pop1_variable STRING, pop2_variable STRING, OUT result STRUCT<t_value FLOAT64, df FLOAT64, p_value FLOAT64> )
BEGIN
EXECUTE IMMEDIATE """
    WITH pop1 AS (
...
BEGIN
  DECLARE result STRUCT<t_value FLOAT64, df FLOAT64, p_value FLOAT64>;
  ...
  ASSERT ROUND(result.t_value, 10) = 25.88834390257;
  ASSERT result.df = 298;
  ASSERT ROUND(result.p_value, 10) = 1.0;
END;
