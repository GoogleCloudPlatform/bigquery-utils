...
usage_data as (
  ...
  -- *****************************************************************
  FROM `committed-use-discount-test.billing.gcp_billing_export_v1_016676_F7EDE9_45E9C2`
  WHERE service.description = "Compute Engine"
  AND CAST(DATETIME(usage_start_time, "America/Los_Angeles") AS DATE) >= "2019-04-01"
),
...
credit_data AS (
  ...
  (
    ...
    INNER JOIN prices ON TRUE
      AND u.sku.id = prices.sku_id
...
SELECT
  ...
  SUM(SUD_cost) AS SUD_cost
FROM cost_data a
FULL OUTER JOIN credit_data b ON TRUE
