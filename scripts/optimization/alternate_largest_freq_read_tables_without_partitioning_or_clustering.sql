...
BEGIN
  ...
  FROM
    `region-us.INFORMATION_SCHEMA.TABLE_STORAGE` s
  JOIN (
    SELECT 
      table_catalog,
      table_schema,
  ...
  ON (s.project_id = c.table_catalog AND s.table_schema = c.table_schema AND s.table_name = c.table_name)
  WHERE
    NOT EXISTS(SELECT * FROM UNNEST(clustering) AS c WHERE c <> "NULL")
    OR NOT EXISTS(SELECT * FROM UNNEST(partitioning) AS p WHERE p = "YES")
  group by s.table_catalog, s.table_schema, s.table_name, table_url, is_clustered, is_partitioned
