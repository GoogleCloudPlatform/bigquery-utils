...
create or replace temp table `optimization_workshop.jobs_referenced_tables` as
...
where report_date > days_to_scan;

-- populate the table
create or replace table `optimization_workshop.tables_access_summary` as
with ref_tables as (
	select 
