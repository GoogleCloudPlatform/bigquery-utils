...
CREATE OR REPLACE FUNCTION
  rs.interval_literal_to_seconds(interval_literal STRING) AS ( /* For intervals in Redshift, there are 360 days in a year. */ (
    SELECT
      SUM(CASE
          ...
          WHEN unit IN ('weeks',
          'week',
          'w' ) THEN num * 60 * 60 * 24 * 7
          WHEN unit IN ('months', 'month' ) THEN num * 60 * 60 * 24 * 30
          WHEN unit IN ('years',
          'year') THEN num * 60 * 60 * 24 * 360
        ELSE
        num
      END
        )
